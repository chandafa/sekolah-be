...
@section('container')
    <div class="col-md-8 offset-md-2 mt-5">
        <a href="{{ route('alert.index', ['token' => $token]) }}" class="btn btn-light border-warning px-4 mb-4"><i
                class="fas fa-arrow-left"></i> Kembali</a>
        <form action="{{ route('alert.store', ['token' => $token]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="nama">Judul alert</label>
                <input type="text" name="nama" id="nama" class="form-control @error('nama') is-invalid @enderror"
                    value="{{ old('nama') }}" placeholder="Besok ada sesuatu..." aria-describedby="nameId">
            ...
            <div class="form-group">
                <label for="alert" class="form-label">Kategori alert</label>
                <select class="form-control @error('id_pemberitahuan_category') is-invalid @enderror"
                    ...
                    @foreach ($alert as $a)
                        @if (old('id_pemberitahuan_category') == $a->id_pemberitahuan_category)
                            <option value="{{ $a->id_pemberitahuan_category }}" selected>
                                {{ $a->pemberitahuan_category_name }}</option>
                        @else
                            <option value="{{ $a->id_pemberitahuan_category }}">{{ $a->pemberitahuan_category_name }}
                            </option>
                        @endif
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <input type="hidden" name="viewer" id="viewer" class="form-control" value="0"
                    aria-describedby="viewId">
                <small id="viewId" class="text-muted d-none"></small>
            </div>
            <div class="form-group">
                <label for="text">Deskripsi alert</label>
                <textarea name="text" id="texteditor" cols="30" rows="10"
                    class="form-control @error('text') is-invalid @enderror" placeholder="Isi dari alert.." aria-describedby="textId">{{ old('text') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input type="text" name="jurnal_by" id="jurnal_by"
                    class="form-control @error('jurnal_by') is-invalid @enderror" value="{{ old('jurnal_by') }}"
                    placeholder="Jurnal By" aria-describedby="jurnalById">
            ...
            <div class="row">
                <div class="col-md-6 py-md-5 py-3">
                    <div class="form-group">
                        <label for="thumbnail">Thumbnail alert</label>
