        ...
        <form action="{{ route('berita.store', ['token' => $token]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama" id="nama" class="form-control @error('nama') is-invalid @enderror"
                    placeholder="Besok ada sesuatu..." aria-describedby="nameId" value="{{ old('nama') }}">
                ...
                <select class="form-control @error('id_pemberitahuan_category') is-invalid @enderror"
                    ...
                    @foreach ($news as $n)
                        @if (old('id_pemberitahuan_category') == $n->id_pemberitahuan_category)
                            <option value="{{ $n->id_pemberitahuan_category }}" selected>
                                {{ $n->pemberitahuan_category_name }}</option>
                        @else
                            <option value="{{ $n->id_pemberitahuan_category }}">{{ $n->pemberitahuan_category_name }}
                            </option>
                        @endif
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea name="text" id="texteditor" cols="30" rows="10"
                    class="form-control @error('text') is-invalid @enderror" placeholder="Isi dari news.." aria-describedby="textId">{{ old('text') }}</textarea>
