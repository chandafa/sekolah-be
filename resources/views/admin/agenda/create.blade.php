        ...
        <form action="{{ route('event.store', ['token' => $token]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="nama" id="nama" class="form-control @error('nama') is-invalid @enderror"
                    value="{{ old('nama') }}" placeholder="Besok ada sesuatu..." aria-describedby="namaId">
            ...
            <div class="form-group">
                ...
                <input type="text" name="target" id="target"
                    ...
                    placeholder="Seluruh Warga Sekolah / Seluruh Kelas XI" aria-describedby="tipeId">
                ...
                <select class="form-control @error('id_pemberitahuan_category') is-invalid @enderror"
                    ...
                    @foreach ($event as $e)
                        @if (old('id_pemberitahuan_category') == $e->id_pemberitahuan_category)
                            <option value="{{ $e->id_pemberitahuan_category }}" selected>
                                {{ $e->pemberitahuan_category_name }}</option>
                        @else
                            <option value="{{ $e->id_pemberitahuan_category }}">{{ $e->pemberitahuan_category_name }}
                            </option>
                        @endif
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea required name="text" id="texteditor" cols="30" rows="10"
                    class="form-control @error('text') is-invalid @enderror" placeholder="Isi dari event.." aria-describedby="textId">{{ old('text') }}</textarea>
