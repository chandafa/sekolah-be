    ...
    <div class="col-md-8 offset-md-2 mt-5">
        ...
        <form action="{{ route('event.update', ['token' => $token, 'event' => $event->id_pemberitahuan]) }}" method="post"
            ...
            <div class="form-group">
                ...
                <input value="{{ $event->nama }}" type="text" name="nama" id="nama"
            ...
            <div class="form-group">
                ...
                <input value="{{ $event->target }}" name="target" id="target"
                    placeholder="Perayaan / Upacara / Classmeet / Istighosah" type="text"
                    class="form-control @error('target') is-invalid @enderror" aria-describedby="tipeId">
                @error('target')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
                ...
                <select class="form-control @error('id_pemberitahuan_category') is-invalid @enderror"
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id_pemberitahuan_category }}"
                            {{ $event->category == $category->id_pemberitahuan_category ? 'selected' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea name="text" id="texteditor" cols="30" rows="10"
                    class="form-control @error('text') is-invalid @enderror" placeholder="Isi dari event.." aria-describedby="textId">{{ $event->text }}</textarea>
                @error('text')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            ...
            <div class="row">
                ...
                <div class="col-md-6 text-center">
                    <img class="w-100 rounded" id="preview" src="{{ asset('img/event/' . $event->thumbnail) }}"
                        alt="">
                </div>
            </div>
            <div class="form-group">
